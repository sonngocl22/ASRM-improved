...
def do_general_random_negative_sampling(args):
    ...
    with open('raw/ASRM/ITEMSDATES.dat', 'rb') as fp:
        item2date = pickle.load(fp)
    with open('raw/ASRM/DATEDICT.dat', 'rb') as fp:
...
def task_prepare_vwfs(args):
    preprocess_asrm(
        args,
        ifeature_fname='feat_wrnd.dat',
        icontext_fname='CXTDict_NoContext.dat',
        rows_fname='data_cxt.txt'
    )
def task_prepare_vwfscxt(args):
    preprocess_asrm(
        args,
        ifeature_fname='feat_wrnd.dat',
        icontext_fname='CXTDict.dat',
        rows_fname='data_cxt.txt'
    )
# for bids

...
class BaseSolver:
    ...
    def load_model(self, purpose: str) -> None:
        ...
        if purpose == 'test':
            ...
            if os.path.isfile(self.check_path):
                ...
                self.best_test_score = check.get('best_test_score', None)
    ...
    def solve(self) -> None:
        ...
        # solve loop
        #modification with or without bids
        if self.mainrel < 1.0:
            if not self.early_stop:
                for epoch in range(self.start_epoch, C['train']['epoch'] + 1):
                    self.solve_train_withbids(epoch, self.mainrel)
    ...
    def solve_train_withbids(self, epoch: int, mainrel: float) -> None:
        self.train_one_epoch_withbids(epoch, mainrel)
    ...
    #added function to train with sales and bids as targets
    def train_one_epoch_withbids(self, epoch: int, mainrel: float) -> None:
        self.logger.info(f"<train one epoch (epoch: {epoch})>")
        ...
        # loop
        pbar = tqdm(self.train_dataloader)
        pbar.set_description(f"[epoch {epoch} train with bids]")
        for sales_batch, bids_batch in pbar:

            # get loss weighted by sales and bids
            sales_loss = self.calculate_loss(sales_batch)
            bids_loss = self.calculate_loss(bids_batch)
            total_loss = sales_loss * mainrel + bids_loss * (1 - mainrel)
            ...
            # step end
            epoch_loss_sum += float(total_loss.data)
            epoch_loss_count += 1
            pbar.set_postfix(loss=f'{epoch_loss_sum / epoch_loss_count:.4f}')
    ...
    def evaluate_valid(self, epoch: int) -> None:
        ...
        # print(f'result_values list: {result_values}')
        # print(f'sum(results_values): {sum(result_values)}')
        # print(f'len(result_values): {len(result_values)}')

        self.logger.info('\t'.join([
            f"[epoch {epoch}]",
            f"valid pivot score: {pivot_score:.4f}"
        ]))
        #added print to capture metrics
        print(f'epoch: {epoch};')
        print(f'valid {pivot}: {pivot_score:.05f};')
    ...
    def evaluate_test(self) -> None:
        ...
        for k in ks:
            ...
            for metric_name in METRIC_NAMES:
                ...
                #added print to capture metrics
                print(f'test {metric_name}@{k}: {result:.05f};')
            reports.append('\t'.join(row))
        #added variable to record best Recall@10 test result
        if self.best_test_score is None or self.best_test_score < results_mean['Recall@10']:
            self.best_test_score = results_mean['Recall@10']
            print(f"new best test Recall@10 score: {results_mean['Recall@10']:.05f}")
        print(f"current best test Recall@10 score is: {self.best_test_score:.05f}")
        ##
        self.logger.info('\n'.join(reports))
